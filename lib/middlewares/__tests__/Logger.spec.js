...
describe('Given the Logger middleware', function () {
  ...
  describe('when using Logger with function and string as a handler', function () {
    it('should log to the console', function () {
      ...
      [machine.name + ': "run" dispatched with payload {},42,hello world', machine.name + ': state changed to "running"', machine.name + ': "stop" dispatched', machine.name + ': state changed to "idle"'].forEach(function (logStr, i) {
  ...
  describe('when using the Logger with a generator function', function () {
    it('should log every step of the generator', function () {
      ...
      [machine.name + ': "run" dispatched with payload {"foo":"bar"},42,hello world', machine.name + ': generator step -> running', machine.name + ': state changed to "running"', machine.name + ': generator step -> {"name":"running"}', machine.name + ': state changed to "running"', machine.name + ': generator step -> {"__type":"call","args":[42]}', machine.name + ': generator step -> running', machine.name + ': state changed to "running"', machine.name + ': "stop" dispatched', machine.name + ': state changed to "idle"'].forEach(function (logStr, i) {
        expect(console.log.getCall(i)).to.be.calledWith(logStr);
      });
    });
  });
